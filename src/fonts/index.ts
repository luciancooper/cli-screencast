...
export default async function createFontCss(
    ...
    fontSpec: string,
): Promise<{ css: string, fontFamily: string, fontColumnWidth: number | undefined } | { css: null }> {
    ...
    for (const [count, k] of embedded.columnWidth) widthMap.set(k, (widthMap.get(k) ?? 0) + count);
    const [fontColumnWidth] = [...widthMap.entries()].sort(([, a], [, b]) => b - a)[0] ?? [];
